...
로컬 카카오 로그인을 시작할 때 `localhost`와 `127.0.0.1`을 섞어 사용하면 브라우저 쿠키가 서로 다른 호스트로 분리되어 OAuth `state` 검증이 실패할 수 있습니다. 서버는 `KAKAO_REDIRECT_URI`가 로컬 주소로 설정된 경우 두 로컬 호스트명을 설정된 호스트명으로 정규화하지만, 테스트할 때는 `http://localhost:8091`로 접속하는 것을 권장합니다. 기존에 실패한 콜백 URL을 새로고침하지 말고 로그인 버튼에서 흐름을 다시 시작하세요.
